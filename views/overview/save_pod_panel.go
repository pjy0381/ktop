...
func NewSavePodPanel(app *application.Application, title string) ui.Panel {
...
func (p *savePodPanel) GetTitle() string {
	return p.title
}

func (p *savePodPanel) Layout(_ interface{}) {
	if !p.laidout {
...
func (p *savePodPanel) DrawHeader(data interface{}) {
	cols, ok := data.([]string)
	if !ok {
...
func (p *savePodPanel) DrawFooter(data interface{}) {}

func (p *savePodPanel) Clear() {
	p.list.Clear()
	p.Layout(nil)
	p.DrawHeader(p.listCols)
}

func (p *savePodPanel) GetRootView() tview.Primitive {
	return p.root
}

func (p *savePodPanel) GetChildrenViews() []tview.Primitive {
	return p.children
}
